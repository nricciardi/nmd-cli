...
impl Builder {
    ...
    pub async fn load_dossier(builder_configuration: &BuilderConfiguration) -> Result<Dossier, BuilderError> {
        ...
        if let Some(dstc) = builder_configuration.documents_subset_to_compile() {
            ...
            dossier = Loader::load_dossier_from_path_buf_only_documents(builder_configuration.input_location(), &dstc, &builder_configuration.codex(), &LoaderConfiguration::default())?;
    ...
    pub async fn build_dossier_compiling_subset(dossier: &mut Dossier, builder_configuration: &BuilderConfiguration, subset_documents_to_parse: Option<HashSet<String>>) -> Result<(), BuilderError> {
        ...
        let compilation_configuration_overlay = Arc::new(RwLock::new(compilation_configuration_overlay));
    ...
    pub async fn watch_compile_dossier(builder_configuration: BuilderConfiguration, min_elapsed_time_between_events_in_secs: u64, preview: Option<Arc<TokioRwLock<HtmlPreview>>>) -> Result<(), BuilderError> {

        let input_location_abs = Arc::new(builder_configuration.input_location().canonicalize().unwrap()); 

        let dossier = Arc::new(TokioRwLock::new(Self::load_dossier(&builder_configuration).await?));
                                    ...
                                    for path in &paths {
                                        ...
                                        if dossier.read().await.configuration().raw_documents_paths().par_iter().find_any(|raw_path| {
                                            ...
                                            document_read_handles.spawn(async move {
                                                ...
                                                let document = Loader::load_document_from_path(&path, &codex, &LoaderConfiguration::default());
